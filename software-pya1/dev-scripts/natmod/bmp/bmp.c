...
static mp_obj_t bmp_load_rgb565(size_t n_args, const mp_obj_t *args, mp_map_t *kw_args) {
    enum { ARG_file, ARG_fb, ARG_scale };
    mp_arg_t allowed_args[] = {
        { MP_QSTR_, MP_ARG_REQUIRED | MP_ARG_OBJ, {.u_obj = MP_OBJ_NULL} },
        { MP_QSTR_, MP_ARG_OBJ, {.u_obj = MP_OBJ_NULL} },
        { MP_QSTR_, MP_ARG_KW_ONLY | MP_ARG_INT, {.u_int = 1} },
    };
    allowed_args[0].qst = MP_QSTR_file;
    allowed_args[1].qst = MP_QSTR_fb;
    allowed_args[2].qst = MP_QSTR_scale;
    ...
    mp_printf(&mp_plat_print, "Details %d %d %d %d Rows %d %d Scale %d\n", width, height, flipped, bpp, row_size, abs_height,scale);
...
static MP_DEFINE_CONST_FUN_OBJ_KW(bmp_load_rgb565_obj, 1, bmp_load_rgb565);
